theory "Rubylog::FileSystemBuiltins", nil do
  ...
  X.file_in(D).if Y.in { d = D.rubylog_resolve_function; Dir["#{d}/*", "#{d}/.*"].sort }.and { not File.directory? Y }.and X.is {File.expand_path Y}
  X.dir_in(D). if Y.in { d = D.rubylog_resolve_function; Dir["#{d}/*", "#{d}/.*"].sort }.and {     File.directory? Y }.and X.is {File.expand_path Y}
